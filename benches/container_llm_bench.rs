...
fn scale_to_min_bytes(mut s: String) -> String {
        ...
        && let Ok(min_bytes) = min_bytes_str.parse::<usize>()
    {
        ...
        if size > 0 && size < min_bytes {
            ...
            let orig = s.clone();
            for _ in 1..repeat { s.push_str(&orig); }
        }
    }
    s
}
...
fn container_llm_bench(c: &mut Criterion) {
    ...
    for &lines in &[1usize, 8, 64] {
        ...
        group.throughput(Throughput::Bytes(input.len() as u64));
        group.bench_with_input(BenchmarkId::new("object_newlines", lines), &input, |b, s| {
            b.iter(|| {
                let out = llm_json::repair_json(s, &opts).unwrap();
                std::hint::black_box(out);
            })
    ...
    group.throughput(Throughput::Bytes(input.len() as u64));
    group.bench_with_input(BenchmarkId::new("valid_json_strict", "fixed"), &input, |b, s| {
        b.iter(|| {
            let out = llm_json::repair_json(s, &opts).unwrap();
            std::hint::black_box(out);
        })
    ...
    let input = scale_to_min_bytes(valid_json());
    group.throughput(Throughput::Bytes(input.len() as u64));
    group.bench_with_input(BenchmarkId::new("valid_json_fastpath", "fixed"), &input, |b, s| {
        b.iter(|| {
    ...
    group.throughput(Throughput::Bytes(input.len() as u64));
    group.bench_with_input(BenchmarkId::new("unicode_comments", "fixed"), &input, |b, s| {
        b.iter(|| {
            let out = llm_json::repair_json(s, &opts).unwrap();
            std::hint::black_box(out);
        })
